% Ce script permet la simulation de chauffage d'une pièce en hiver avec une
% température extérieure à -10°C, une température à la porte de 15°C, 
% et le chauffage apportant de la chaleur à une puissance de 481
...
% Chauffage 2
%H = ((X>-0.8) & (X<-0.5) & (Y>0.6) & (Y<0.9)) | ...     % chauffage dans le coin ouest-sud
    %((X>-0.6) & (X<-0.3) & (Y>-0.9) & (Y<-0.65));       % chauffage salle de bain sur le mur nord
...
tic
while (mean(mean(u(G(G>0)))) < 21) % On boucle jusqu'à atteindre une température moyenne de la pièce de 21°C
    u_old = u;
