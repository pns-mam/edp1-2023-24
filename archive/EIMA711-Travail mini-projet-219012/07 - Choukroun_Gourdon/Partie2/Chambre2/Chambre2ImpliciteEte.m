...
%{
Code permettant la simulation de refroidissement d'une pièce en été avec une température extérieure à 35°C
une température à la porte de 20 °C, et le chauffage (en mode clim ici) apportant du froid à une puissance de 150 
On utilise ici le schéma implicite.
%}
...
tic
cpt=0
while (mean(mean(u(G(G>0)))) > 22)
    cpt=cpt+1;
    u_old = u;
    u = (I-nu*dt2*A)\(u_old - nu*dt2*b);
    U = G;
    U(G>0) = full(u(G(G>0)));       
    mesh(X,Y,U);                         
    axis('ij'); 
...
disp("Température méthode implicite "+mean(mean(u(G(G>0)))));
