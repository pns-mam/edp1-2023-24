function Chambre1(ot,dt,ht,n)
% Chambre1 calcule la température ambiante dans une chambre
% U = Chambre1(ot, dt, ht, n); prend en paramètres la température extérieure ot,
...
G = ((X>0.2) & (X<0.8) & (Y>-0.6) & (Y<0.2)) | ...  % geometrie de la chambre
...
door = [];
window=[];                          % indices correspondant a la porte
for i = 2:n-1                       % ajout de CL Neumann
  for j = 2:n-1                     % pour les murs isolants
    ind = G(i,j);
    if ind~=0
      if G(i,j-1) == 0             
        if (Y(i,j)>0.2 && Y(i,j)<0.6)  % On garde la CL Dirichlet pour la fenetre
          window = [window G(i,j)];
        else
