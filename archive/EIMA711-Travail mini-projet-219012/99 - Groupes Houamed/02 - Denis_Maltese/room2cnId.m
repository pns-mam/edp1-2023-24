function U = room2cnId(ot,dt,ht,hdt,n,nu,alpha,eps,tinit,tid)
  %{
Calcul de la solution itérative du problème de la partie 2 avec
température idéale dans une zone donnée pour notre chambre 2
en utilisant la méthode de Crank-Nicolson avec:
...
alpha: nombre de cfl
eps: tolérance de convergence, valeur de la norme de u-uold
en dessous de laquelle on considère que le modèle a convergé
tinit:température uniforme dans la pièce avant le début de
la simulation(°C)
tid: la température considérée commme idéale que l'on veut approcher dans la
zone I(°C)
%}
...
I=((X>-1) & (X<-0.2) & (Y>0.3) & (Y<1));         %zone où on veut tid
...
for l = 1:2000          % boucle en temps 
    uold = u;
    u = (eye(size(A))-((nu*deltat)/2)*A)\((eye(size(A))+((nu*deltat)/2)*A)*uold - nu*deltat*b); % Crank-Nicolson
    U = G;
    U(G>0) = full(u(G(G>0)));             % on met la solution dans une matrice
    mesh(X,Y,U);                          % on dessine la solution sur la grille
    axis('ij');
    if abs(mean(u(I))-tid)<eps            %condition de convergence
      disp("convergence vers la température idéale au pas de temps:")
      disp(l)
      return
    end
    pause(0.01)
