...
for k = [-1 m 1 -m]
   ...
   q = find(Q);                 %on trouve tous les voisins qui sont non nuls dans la direction k
   % Connect interior points to neighbors with -1's.
   i = [i; G(p(q))];            %identique a [i;q], ici il s'agit des emplacements
   j = [j; Q(q)];               %ici il s'agit de leur valeur
   s = [s; -ones(length(q),1)];
end
D = sparse(i,j,s);              %renvoie (i,j)=s