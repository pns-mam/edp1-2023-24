...
%H = ((X>-0.2) & (X<0.2) & (Y>0.5) & (Y<0.75));      % position du chauffage
                                                     % sur le mur du fond
...
door = [];                          % indices correspondants a la porte
window = [];                        % indices correspondants à la fenêtre
...
temp = sum(sum(U))/length(k);       % calcul de la température ambiante 
mesh(X,Y,U);                        % correspondant a la grille 
axis('ij');                         % on dessine la solution sur la grille
title('Chambre 1 avec chauffage au niveau de la fenetre');
toc
